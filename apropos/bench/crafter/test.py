...
class SimpleVisionLanguageAgent:
        ...
        def render_history(self):
            react_history = [f"<{i} reasoning step(s) in the past>{item}</{i} reasoning step(s) in the past>" for i, item in enumerate(reversed(self.react_history[-5:]), 1)]
            obs_history = [f"<{i} environment step(s) in the past>{item['text']}</{i} environment step(s) in the past>" for i, item in enumerate(reversed(self.obs_history[-5:]), 1)]
            return "\n".join(react_history), "\n".join(obs_history)
...
Your next actions / thought: """
            ...
            for obs in visual_recent_obs:
                if isinstance(obs, bytes):
                    ...
                    img_array = img_array.reshape((64, 64, 3))  # Assuming 64x64x3 image size
        ...
        async def add_observations(self, observations: List[Dict]):
            for observation in observations:
                self.obs_history.append({"text": observation["text_obs"][0].visual_readout(), "image": observation["vis_obs"]})
...
async def score_rollouts(k_steps=10, n_rollouts=10, verbose=False, model_name="gpt-4o-mini", modality: Literal["text","vision"]="text"):
...
if __name__ == "__main__":
    ...
    hafner_score,achievement_probs, total_price = asyncio.run(
        score_rollouts(k_steps=300, n_rollouts=1, verbose=True, model_name="gpt-4o-mini-2024-07-18",modality="text")#hermes-3-llama-3.1-405b-fp8-128k
    )
