...
from bench.base import QABenchmark, Question
...
def josh_math_metric(gold, pred, trace=None):
...
class MATH_Question(Question):
    ...
    def compute_and_score_attempt_sync(self, lm_dag: LM_DAG) -> Tuple[bool, DagRecord]:
        ...
        assert unique_inputs[0] == "<<<MATHEMATICS_QUESTION>>>", f"The input edge should be for the question, instead got {unique_inputs[0]}"
        ...
        score = josh_math_metric(self.information['answer'], answer)
    ...
    async def compute_and_score_attempt(self, lm_dag: LM_DAG, frozen_params: Dict = {"dag_record": None, "unfrozen_node_names": []}) -> Tuple[bool, DagRecord]:
        ...
        output, dag_record = await lm_dag.arun({"question": self.information['question']},frozen_params=frozen_params, verbose=True)
        answer = output["answer"]
        score = josh_math_metric(self.information['answer'], answer)
        return score, dag_record
...
class HendryksMath_Benchmark(QABenchmark):
    ...
    def __init__(self):
        ...
        train = [standardize_hendryks_question(hf_dict) for hf_dict in list(dataset['train'])]
        test = [standardize_hendryks_question(hf_dict) for hf_dict in list(dataset['test'])]
        train = [{
            'question': info['question'],
            'answer': info['answer'],
            'gold_reasoning': info['gold_reasoning'],
            'topic': info['topic']
        } for info in train if info and info['topic']]
        test = [{
            'question': info['question'],
            'answer': info['answer'],
            'gold_reasoning': info['gold_reasoning'],
            'topic': info['topic']
        } for info in test if info]

        random.seed(42)
        random.shuffle(train)
        random.shuffle(test)
        self.train = [MATH_Question(info) for info in train[:len(train)//3]]
        self.dev = [MATH_Question(info) for info in train[len(train)//3:2*len(train)//3]]
        self.test = [MATH_Question(info) for info in test]

        print("Size of train, dev, test:", len(self.train), len(self.dev), len(self.test))
